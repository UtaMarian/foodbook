...
/**
 * Pe un telefon real "localhost" e telefonul insusi, nu laptopul.
 * Luam IP-ul din adresa serverului Metro, deci merge fara configurare.
 */
function resolveBaseUrl(): string {
  ...
  return host ? `http://${host}:3000/v1` : 'http://localhost:3000/v1';
}
...
export async function request<T>(path: string, options: RequestOptions = {}): Promise<T> {
  ...
  try {
    ...
    // Mesajul din ApiError e generic pentru utilizator; eroarea reala ramane
    // in terminalul Expo, utila la depanare (retea, CORS, FormData etc.).
